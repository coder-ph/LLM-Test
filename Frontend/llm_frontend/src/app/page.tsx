    ...
    try {
      const res = await fetch('http://localhost:8000/api/v1/query', {
             ...
             if (!res.ok) {
              ...
              throw new Error(errorData.detail || 'An unknown error occured, please try again')
             }
      ...
      setError(error.message || 'Failed to get a response from AI');
...
return (
  <div className="font-sans flex flex-col min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
    ...
    <main className='flex-1 overflow-y-auto pt-20 pb'
  </div>
)